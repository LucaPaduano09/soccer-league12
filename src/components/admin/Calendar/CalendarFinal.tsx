...
const CalendarFinal = () => {
    ...
    const handleCreateDay = async (e) => {
      e.preventDefault();
      const response = await fetch(
        "https://soccer-league12.herokuapp.com/calendarFinal/add",
        {
          method: "POST",
    ...
    const handleAddGameToDay = async (gameId) => {
      if (selectedDayGames?.length > 0) {
        ...
        if (add[0].partite === null) {
          const response = await fetch(
            "https://soccer-league12.herokuapp.com/calendarFinal-first/" +
              giornataToFind,
            {
              method: "POST",
        ...
        if (add[0].partite.length !== null) {
          const response = await fetch(
            "https://soccer-league12.herokuapp.com/calendarFinal/" + giornataToFind,
            {
              method: "POST",
              body: JSON.stringify({ partite: gameToAddId }),
              mode: "cors",
    ...
    const handleRemoveGame = async (gameId) => {
      ...
      if (remove[0].partite.length === 1) {
        const response = await fetch(
          "https://soccer-league12.herokuapp.com/calendarFinal-remove-first/" +
            giornataToFind,
          {
            method: "POST",
            mode: "cors",
      ...
      if (remove[0].partite.length > 1) {
        const response = await fetch(
          "https://soccer-league12.herokuapp.com/calendarFinal-remove/" +
            giornataToFind,
          {
            method: "POST",
    ...
    const handleRemoveDay = async (e) => {
      ...
      if (calendar?.length > 0) {
        ...
        if (searchedDay?.length > 0) {
          const response = await fetch(
            "https://soccer-league12.herokuapp.com/calendarFinal-remove-day",
            {
              method: "DELETE",
              mode: "cors",
