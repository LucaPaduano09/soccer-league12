...
const Classifica = () => {
  ...
  const sorted =
    teamsFiltered !== undefined && teamsFiltered !== null
      ? teamsFiltered.sort((a, b) => (a.points > b.points ? -1 : 1))
      : "";

  const sortedFinale =
    teamsFiltered !== undefined && teamsFiltered !== null
      ? teamsFiltered.sort((a, b) =>
          a.points_final > b.points_final
            ? -1
            : a.points_final === b.points_final &&
              a.goal_fatti_final > b.goal_fatti_final
            ? -1
            : 1
        )
      : "";
  ...
  const getTeamGoal = (teamId) => {
    if (players !== null && players !== undefined && players.length > 0) {
      if (girone !== "Finale") {
        let correctPlayers = players.filter(
          (player: any) => player.teamId === teamId
        );
        let goals = 0;
        ...
        correctPlayers.forEach((cp: any) => {
          let scoresFinal =
            cp.scores_final !== null && cp.scores_final !== undefined
              ? cp.scores_final
              : 0;
          goals += scoresFinal;
        });
  ...
  return (
    <>
      <div className="Classifica__container">
        <div className="Classifica__container__slider">
          <button
            className={
              ...
              (scoreActive ? "__active" : "__noActive")
            }
            onClick={() => handleScoreActive()}
          >
            Vittorie
          </button>
          <button
        ...
        {teams.length === 0 && (
          <p>
            In questa sezione potrai vedere la classifica aggiornata di Qatar
            2022 by BeFootballStar
          </p>
        )}
                  ...
                  sortedFinale?.map((team, index) => (
                    <tr>
                      ...
                      <td>
                        ...
                        team.goal_fatti_final !== undefined
                          ? team.goal_fatti_final
                          : 0}
                      </td>
                      <td>
                        {team.goal_subiti_final !== null &&
                        team.goal_subiti_final !== undefined
                          ? team.goal_subiti_final
                          : 0}
                      </td>
                      <td>
                        {team.goal_subiti_final !== null &&
                        team.goal_subiti_final !== undefined
                          ? team.goal_fatti_final - team.goal_subiti_final
                          : 0}
                      </td>
                    </tr>
                  ))}
